...
/* ============================================================
   SALVAR VALORES FINANCEIROS (saldo, renda, limite)
============================================================ */
if ($_SERVER['REQUEST_METHOD'] === 'POST') {

    $saldo = $_POST['saldo_inicial'] ?? 0;
    $renda = $_POST['renda_prevista'] ?? 0;
    $limite = $_POST['limite_gastos'] ?? 0;

    // Verifica se já existe registro para o usuário
    $check = $pdo->prepare("SELECT id_usuario FROM valores_usuarios WHERE id_usuario = ?");
    ...
    if ($check->rowCount() > 0) {
        ...
        $update = $pdo->prepare("
            ...
            WHERE id_usuario = ?
        ");
        $update->execute([$saldo, $renda, $limite, $id_usuario]);
    } else {
        // Cria caso não exista
        $insert = $pdo->prepare("
            INSERT INTO valores_usuarios (id_usuario, saldo_inicial, renda_prevista, limite_gastos)
            VALUES (?, ?, ?, ?)
        ");
        $insert->execute([$id_usuario, $saldo, $renda, $limite]);
    }
...
if ($porcentagem >= 50) {
    ...
    if ($porcentagem >= 95) {
        ...
        $notifTexto = "Gastou R$ " . number_format($gastoMes, 2, ',', '.') .
...
<body class="bg-gray-100 dark:bg-gray-900 flex text-gray-900 dark:text-gray-100 transition-colors duration-300">
    ...
    <div class="flex-1 flex flex-col">
        <header
            ...
            <h2 class="text-xl font-bold">Configurações de Valores</h2>
